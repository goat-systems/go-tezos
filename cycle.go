...
func (t *GoTezos) Cycle(cycle int) (Cycle, error) {
	...
	if cycle > head.Metadata.Level.Cycle+t.Constants.PreservedCycles-1 {
		return Cycle{}, errors.Errorf("could not get cycle '%d': request is in the future", cycle)
	}

	var c Cycle
	if cycle < head.Metadata.Level.Cycle {
		block, err := t.Block(cycle*t.Constants.BlocksPerCycle + 1)
		if err != nil {
			return Cycle{}, errors.Wrapf(err, "could not get cycle '%d'", cycle)
		}
	...
	level := ((cycle - t.Constants.PreservedCycles - 2) * t.Constants.BlocksPerCycle) + (c.RollSnapshot+1)*t.Constants.BlocksPerRollSnapshot
	if level < 1 {
		level = 1
	}
...
func (t *GoTezos) getCycleAtHash(blockhash string, cycle int) (Cycle, error) {
	...
	if err != nil {
		return c, errors.Wrap(err, "could not unmarshal")
	}
